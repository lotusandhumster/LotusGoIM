<template>
    <el-text>私聊消息管理</el-text><br/>
    <el-input style="width: 20%;" v-model="searchFilter.senderId" placeholder="发送者ID">
      <template #prepend>
        发送者ID
      </template>
    </el-input>
    <el-input style="width: 20%;" v-model="searchFilter.receiverId" placeholder="接收者ID">
      <template #prepend>
        接收者ID
      </template>
    </el-input>
    ...
    <el-table stripe :data="messages">
      <el-table-column prop="privateMessageId" label="ID"/>
      ...
      <el-table-column label="操作">
        <template #default="scope">
          <el-button type="danger" plain @click="deleteMessage(scope.row.privateMessageId)"><el-icon><Delete/></el-icon></el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { PrivateMessageModel, PrivateMessageSearchFilter } from '../../types'
...
const messageApi = usePrivateMessageApi(that)
...
const messages = ref<PrivateMessageModel[]>([])
...
const resetForm = () => {
    searchFilter.value.senderId = undefined
    searchFilter.value.receiverId = undefined
}
const findMessages = () => {
